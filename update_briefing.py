...
"""
오늘의 브리핑 자동 업데이트
- Claude API (claude-haiku-4-5) 사용 — 안정적, 저렴
...
def claude(prompt, max_tokens=300):
    ...
    try:
        r = requests.post(CLAUDE_URL, headers=CLAUDE_HDRS, json=payload, timeout=20)
...
규칙: 줄 앞에 기호 없이 내용만, 한 줄 40자 이내, 수치·사실 위주, 한국어, 딱 3줄만 출력"""
    text = claude(prompt, max_tokens=150)
    if not text:
        return None
    ...
    for source, src_class, url in sources:
        for item in fetch_rss(source, src_class, url):
            ...
            if key not in seen:
                ...
                time.sleep(0.5)   # Claude API는 빠름 — 딜레이 최소화
        if len(news) >= 5:
            break
...
def make_infographic(titles):
    ...
    prompt = f"""오늘({today}) 뉴스 제목들:
{titles_str}

위 뉴스를 분석해서 아래 JSON만 출력해 (다른 텍스트 없이):
{{
  "핵심수치": [
    {{"label": "라벨(5자이내)", "value": "숫자+단위", "desc": "설명(10자이내)"}},
  ...
  "주요이슈": [
    {{"title": "이슈제목(12자이내)", "desc": "설명(28자이내)", "tag": "분야"}},
    {{"title": "이슈제목", "desc": "설명", "tag": "분야"}},
    {{"title": "이슈제목", "desc": "설명", "tag": "분야"}}
  ]
}}"""
    text = claude(prompt, max_tokens=500)
...
def render_infographic(data):
  ...
  <div class="icard">
    ...
    <div style="font-size:30px;font-weight:800;color:var(--red);margin:12px 0">{n["value"]}</div>
  </div>'''
  ...
  <div class="icard">
    ...
    <p style="font-size:13px;color:var(--text);margin:10px 0 0">{iss["desc"]}</p>
  </div>'''
...
for section, items in section_news.items():
    if items: new_news += make_section(section, items)
news_block = f"<!-- AUTO_NEWS_START -->{new_news}\n    <!-- AUTO_NEWS_END -->"
...
if all_titles and "<!-- AUTO_INFO_START -->" in html:
    ...
    if info_data:
        info_html = render_infographic(info_data)
